...
User runs `/belief analyze` or asks to review client materials, ingest documents, analyze homework, audit what's been uploaded, or check for gaps in client materials.
...
- If no knowledge base exists, prompt the user to run `/belief intake` first
...
**Gemini meeting notes**: These are Google Docs with names like `[Client] BELIEF Session - [date] - Notes by Gemini`. Read as Google Docs. Flag these separately — they contain session content that feeds `/belief synthesize`.
...
3. Update the "Last Updated" field: `[today's date] by /belief analyze`
...
> "Would you like to:
> - Run `/belief research` to fill gaps with independent market research?
