...
Accumulates facilitation patterns, industry intelligence, and process improvements across all engagements. Read by `/belief:3-prep` to enhance playbooks with cross-engagement intelligence.
...
The homework document sent to clients 3 weeks prior covers these areas. Use this to audit homework completeness in `/belief:2-analyze`:
...
Use this checklist in `/belief:2-analyze` to score materials completeness and identify gaps.
...
The `/belief:1-intake` command should prompt for section ownership assignments.
...
The plugin's `/belief:2-analyze` and `/belief:3-prep` commands should produce outputs that are strictly superior to these prompts — more context-aware, more nuanced, and building on the full knowledge base rather than a single paste.
...
- `/belief:1-intake` creates the doc and populates Engagement Config
- `/belief:2-analyze` populates Document Inventory, Synthesized Understanding
- `/belief:2-research` populates Research Findings
- `/belief:3-prep` reads everything, writes the playbook as a separate doc
- `/belief:4-synthesize` populates Session Insights
- `/belief:5-distill` populates Brand Foundation Draft
- `/belief:6-learn` reads the full KB, writes to the cross-client KB
...
The `/belief:1-intake` command creates this folder if it doesn't exist and populates it with:
